...
const MultipartParser = class MultipartParser {
	...
	_handleFile({ fieldname, file, filename, incoming }) {
		return new Promise((resolve, reject) => {
			...
			// 8 keeps the peak concurrent GCS API calls below the default
			// EventEmitter maxListeners threshold of 10.
			...
			file.once("limit", () => {
				this._log.warn(
					`multipart - Uploaded package exceeds the max allowed file size - Field: ${fieldname} - Filename: ${filename} - Name: ${incoming.name} - Version: ${incoming.version} - Type: ${incoming.type} - Limit: ${this._pkgMaxFileSize} bytes`,
				);
				ac.abort();
