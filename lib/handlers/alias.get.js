...
const AliasGet = class AliasGet {
	...
	async handler(req, type, name, alias, extra) {
		...
		// Reject extras that contain path traversal sequences or encoded
		// characters that could escape the package path after decoding.
		if (pExtra) {
			let fullyDecoded;
			try {
				fullyDecoded = decodeURIComponent(pExtra);
			} catch {
			...
			if (
				...
				fullyDecoded.includes("\\") ||
				fullyDecoded.includes("//") ||
				fullyDecoded.includes("%2F") ||
				fullyDecoded.includes("%2f") ||
				fullyDecoded.includes("%5C") ||
				fullyDecoded.includes("%5c")
			) {
				this._log.debug(
					`alias:get - Path traversal detected in extras - Pathname: ${pExtra}`,
				);
				const e = new HttpError.NotFound();
				end({ labels: { success: false, status: e.status } });
				throw e;
