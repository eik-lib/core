...
const { createFilePathToPackage, createFilePathToAsset, createFilePathToVersion } = require('../utils/path-builders-fs');
...
const PkgPut = class PkgPut {
        ...
        return new Promise((resolve, reject) => {
                ...
                const extract = new tar.Parse({
                    onentry: (entry) => {
                        ...
                        const promise = new Promise(async (res, rej) => {
                            ...
                            pipeline(entry, hasher, writer, error => {
                                if (error) {
                                ...
                                asset.integrity = `sha512-${hasher.digest('base64')}`;
            ...
            busboy.once('completed', (fieldname, filename) => {
                Promise.all(resolver).then(assets => {
                    ...
                    this._log.info(`pkg:put - Successfully wrote package meta file to sink - Pathname: ${path}`);
    ...
    async _readVersions (incoming) {
        ...
        try {
            ...
            this._log.info(`pkg:put - Successfully read version meta file from sink - Pathname: ${path}`);
        } catch (error) {
            // File does not exist, its probably a new package
            versions = new Versions(incoming);
            this._log.info(`pkg:put - Version meta file did not exist in sink - Create new - Pathname: ${path}`);
        }
        return versions;
    }
    ...
    async _writeVersions (incoming, versions) {
        ...
        this._log.info(`pkg:put - Successfully wrote version meta file to sink - Pathname: ${path}`);
    }
    ...
    async handler (req, org, name, version) {
        ...
        if (!versions.check(version)) {
            this._log.info(`pkg:put - Semver version is lower than previous version of the package - Org: ${org} - Name: ${name} - Version: ${version}`);
