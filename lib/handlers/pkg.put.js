...
const PkgPut = class PkgPut {
	...
	async _writeEikJson(incoming, capturedEikJson) {
		...
		const buffer = await writeJSONWithOptions(
			this._sink,
			eikJsonPath,
			content,
			"application/json",
		...
		this._log.info(
			`pkg:put - Successfully wrote eik.json to sink - Pathname: ${eikJsonPath}`,
		);
	...
	/**
	 * @param {any} incoming
	 * @returns {Promise<{ versions: InstanceType<typeof Versions>, generation: string }>}
	 */
	async _readVersions(incoming) {
		...
		try {
			...
			return { versions: new Versions(incoming), generation: "" };
		}
		...
		const { data, generation } = await readJSONWithGeneration(this._sink, path);
		this._log.info(
			`pkg:put - Successfully read version meta file from sink - Pathname: ${path}`,
		);
		return { versions: new Versions(data), generation };
	}
	...
	/**
	 * Writes versions.json with compare-and-swap semantics when a generation
	 * token is available. On conflict the read-modify-write cycle is retried.
	 *
	 * @param {any} incoming
	 * @param {InstanceType<typeof Versions>} versions
	 * @param {string} generation
	 * @param {string} pVersion
	 * @param {string} integrity
	 */
	async _writeVersions(incoming, versions, generation, pVersion, integrity) {
		const path = createFilePathToVersion(incoming);
		const maxRetries = 3;
		/** @type {{ ifGenerationMatch?: string | number }} */
		let options = generation ? { ifGenerationMatch: generation } : {};
		let currentVersions = versions;
		for (let attempt = 0; attempt <= maxRetries; attempt++) {
			try {
				await writeJSONWithOptions(
					this._sink,
					path,
					currentVersions,
					"application/json",
				...
				if (err.code === "CONFLICT" && attempt < maxRetries) {
					this._log.info(
						`pkg:put - versions.json conflict on attempt ${attempt + 1}, retrying`,
					);
					const { versions: fresh, generation: newGen } =
						await this._readVersions(incoming);
					fresh.setVersion(pVersion, integrity);
					currentVersions = fresh;
					options = newGen ? { ifGenerationMatch: newGen } : {};
					continue;
				}
				throw error;
			}
	...
	async handler(req, user, type, name, version) {
		...
		const { versions, generation } = await this._readVersions(incoming).catch(
			() => {
		...
		try {
			...
			if (err.code === "ALREADY_EXISTS") {
				// Another replica committed this version while this upload was in
				// flight. Treat it as a conflict identical to the early existence check.
				const e = new HttpError.Conflict();
				end({ labels: { success: false, status: e.status, type } });
				throw e;
			}
		...
		try {
			...
			this._log.info(
				`pkg:put - Successfully wrote package meta file to sink - Pathname: ${pkgMetaPath}`,
			);
			// eslint-disable-next-line no-unused-vars
		} catch (error) {
			const e = new HttpError.BadGateway();
