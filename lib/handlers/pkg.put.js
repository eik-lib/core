...
const Parser = class Parser extends Duplex {
    constructor(incoming, sink) {
        ...
        this._parser.on('file', (fieldname, file, filename, encoding, mimetype) => {
            ...
            pipeline(file, extract, async (error) => {
                ...
                try {
                    await utils.writeJSON(sink, this._log.path, this._log, 'application/json');
...
const handler = async (sink, req, org, name, version) => {
    ...
    return await init(sink, incoming);
};
