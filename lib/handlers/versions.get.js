...
const VersionsGet = class VersionsGet {
	...
	async handler(req, type, name) {
		...
		// Check existence first to distinguish a genuinely missing package
		// from a transient infrastructure error. Without this distinction,
		// any GCS or network failure would surface as 404, which callers
		// may misinterpret as "package does not exist".
		try {
			await this._sink.exist(path);
		} catch {
			this._log.debug(
				`pkg:latest - Package versions not found - Pathname: ${path}`,
			);
			const e = new HttpError.NotFound();
			end({ labels: { success: false, status: e.status, type } });
			throw e;
		...
		// File exists — any read failure here is an infrastructure error.
		let file;
		try {
			file = await this._sink.read(path);
		} catch (error) {
			this._log.error(
				`pkg:latest - Failed to read package versions from sink - Pathname: ${path}`,
			);
			...
			if (outgoingStream) {
				outgoingStream.on(
					...
					(/** @type {NodeJS.ErrnoException} */ err) => {
						// ERR_STREAM_PREMATURE_CLOSE means the client closed the
						// connection before the transfer finished. This is normal
						// browser behaviour (navigation, cancelled requests, etc.)
						// and not a server-side error worth alerting on.
						if (err.code === "ERR_STREAM_PREMATURE_CLOSE") {
							this._log.debug(`pkg:latest - File stream error - ${err}`);
						} else {
		...
		this._log.debug(`pkg:latest - Package versions found - Pathname: ${path}`);
