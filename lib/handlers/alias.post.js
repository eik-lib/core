...
const AliasPost = class AliasPost {
	...
	async handler(req, user, type, name, alias) {
		...
		try {
			...
			this._log.info(
				`alias:post - Validation failed - ${error instanceof Error ? error.message : String(error)} - TraceId: ${traceId}`,
			);
		...
		if (!org) {
			this._log.debug(
				`alias:post - Hostname does not match a configured organization - ${url.hostname} - TraceId: ${traceId}`,
			);
		...
		if (!exist) {
			this._log.info(
				`alias:post - Alias does not exists - Org: ${org} - Type: ${type} - Name: ${pName} - Alias: ${pAlias} - TraceId: ${traceId}`,
			);
			const e = new HttpError.NotFound();
			end({ labels: { success: false, status: e.status, type } });
			throw e;
		...
		this._log.info(
			`alias:post - Alias created - Org: ${org} - Type: ${type} - Name: ${pName} - Alias: ${pAlias} - Version: ${resolvedAlias.version} - TraceId: ${traceId}`,
		);
