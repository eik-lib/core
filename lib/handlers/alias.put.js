...
const AliasPut = class AliasPut {
			...
			this._multipart
				...
				.then(async (alias) => {
					try {
						...
						this._log.error(
							`alias:put - Unable to locate requested published package version - Version ${alias.name}@${alias.version}`,
						);
						this._log.trace(error);
	...
	async handler(req, user, type, name, alias) {
		...
		try {
			...
			this._log.info(
				`alias:put - Validation failed - ${error instanceof Error ? error.message : String(error)} - TraceId: ${traceId}`,
			);
		...
		if (!org) {
			this._log.debug(
				`alias:put - Hostname does not match a configured organization - ${url.hostname} - TraceId: ${traceId}`,
			);
			const e = new HttpError.BadRequest();
		...
		if (exist) {
			this._log.info(
				`alias:put - Alias exists - Org: ${org} - Type: ${type} - Name: ${pName} - Alias: ${pAlias} - TraceId: ${traceId}`,
			);
		...
		const resolvedAlias = await this._parser(incoming);

		this._log.info(
			`alias:put - Alias updated - Org: ${org} - Type: ${type} - Name: ${pName} - Alias: ${pAlias} - Version: ${resolvedAlias.version} - TraceId: ${traceId}`,
		);
