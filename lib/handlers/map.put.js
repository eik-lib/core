...
const MapPut = class MapPut {
	...
	async _readVersions(incoming) {
		...
		try {
			...
			// File does not exist — this is a new import map.
			this._log.info(
		...
		// File exists. Any failure here must propagate rather than be
		// swallowed — silently falling back to an empty Versions object
		// would cause _writeVersions to overwrite the index and erase the
		...
		for (let attempt = 0; attempt <= maxRetries; attempt++) {
			try {
				...
				if (err.code === "CONFLICT" && attempt < maxRetries) {
					this._log.info(
						`map:put - versions.json conflict on attempt ${attempt + 1}, retrying`,
					);
