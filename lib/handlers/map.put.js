...
const MapPut = class MapPut {
	...
	async _readVersions(incoming) {
		...
		try {
			...
			this._log.info(
				`map:put - Version meta file did not exist in sink - Create new - Pathname: ${path} - TraceId: ${traceId}`,
			);
		...
		this._log.info(
			`map:put - Successfully read version meta file from sink - Pathname: ${path} - TraceId: ${traceId}`,
		);
	...
	async _writeVersions(incoming, versions) {
		...
		this._log.info(
			`map:put - Successfully wrote version meta file to sink - Pathname: ${path} - TraceId: ${traceId}`,
		);
	}
	...
	async handler(req, user, name, version) {
		...
		try {
			...
			this._log.info(
				`map:put - Validation failed - ${error instanceof Error ? error.message : String(error)} - TraceId: ${traceId}`,
			);
		...
		if (!org) {
			this._log.debug(
				`map:put - Hostname does not match a configured organization - ${url.hostname} - TraceId: ${traceId}`,
			);
			const e = new HttpError.BadRequest();
		...
		if (!versions.check(pVersion)) {
			this._log.info(
				`map:put - Semver version is lower than previous version of the package - Org: ${org} - Name: ${pName} - Version: ${pVersion} - TraceId: ${traceId}`,
			);
